...
import { type CreationResult, type FetchResult, file_schema } from '~/types'
...
export async function get_patient_documents(
	patient_id: number,
): Promise<FetchResult<PatientDocumentModel>> {
	const db_file_records = await db
		.select()
		.from(patient_documents)
		.where(eq(patient_documents.patient_id, patient_id))
	return {
		success: true,
...
export async function get_document_by_id(
	doc_id: number,
): Promise<FetchResult<DocumentWithBuffer>> {
	const db_file_records = await db
		.select()
		.from(patient_documents)
		.where(eq(patient_documents.id, doc_id))
	const files = (
		await Promise.all(
